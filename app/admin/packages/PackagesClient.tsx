...
export function PackagesClient({ initialPackages }: { initialPackages: MergedPackage[] }) {
  ...
  const toggleEnabled = (pkg: MergedPackage) => {
    ...
    updatePackage(pkg.id, { enabled: !pkg.enabled, priceOverride: pkg.priceOverrideActive ? pkg.price : null });
  };
  ...
  const savePrice = (pkg: MergedPackage) => {
    ...
    setPackages((prev) =>
      prev.map((p) => (p.id === pkg.id ? { ...p, price: val, priceOverrideActive: true } : p))
    );
  ...
  const clearPrice = (pkg: MergedPackage) => {
    setPackages((prev) =>
      prev.map((p) =>
        p.id === pkg.id ? { ...p, price: initialPackages.find((ip) => ip.id === p.id)!.price, priceOverrideActive: false } : p
      )
    );
          ...
          {packages.map((pkg) => {
            ...
            return (
              <tr key={pkg.id} className={`hover:bg-gray-50 ${!pkg.enabled ? "opacity-50" : ""}`}>
                ...
                <td className="px-4 py-4">
                  {pkg.priceFromTime ? (
                    <span className="text-xs text-gray-400">Hourly — not overridable</span>
                  ) : isEditingThis ? (
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onKeyDown={(e) => { if (e.key === 'Enter') savePrice(pkg); if (e.key === 'Escape') setEditingPrice(null); }}
                        autoFocus
                      ...
                      <button onClick={() => setEditingPrice(null)} className="text-xs text-gray-500 hover:text-gray-900">
                        Cancel
                      </button>
                    </div>
                  ) : pkg.priceOverrideActive ? (
                    <div className="flex items-center gap-2">
                ...
                <td className="px-4 py-4">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${pkg.enabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-500"}`}>
