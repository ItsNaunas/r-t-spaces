...
const CarouselControl = ({
  ...
  return (
    <button
      className={`w-10 h-10 flex items-center mx-2 justify-center bg-[var(--primary)] border-2 border-[var(--plum)] focus:border-[var(--gold)] focus:outline-none hover:bg-[var(--gold)] hover:border-[var(--plum)] hover:-translate-y-0.5 active:translate-y-0.5 transition duration-200 ${
        type === "previous" ? "rotate-180" : ""
      }`}
      title={title}
      onClick={handleClick}
    >
      <IconArrowNarrowRight className="text-[var(--plum)]" />
    </button>
  );
};
